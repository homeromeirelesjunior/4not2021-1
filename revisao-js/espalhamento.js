...
function somaTudo(...nums) {
    let soma = 0
    for (let number of nums) soma += number
    return soma
}
...
function calcular (operação, ...nums) {
    let resultadoo
    switch (operação) {
        case '+':
            // resultado = 0
            // for (let number of numbs) soma += number
            resultado = somaTudo(...nums)
            break
        
        case '*':
            resultado = 1
            for (let number of nums) resultado *= number
    }
    return resultado
}
